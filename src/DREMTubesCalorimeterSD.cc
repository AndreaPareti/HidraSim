...
/// \brief Implementation of the DREMTubesCalorimeterSD class
...
void DREMTubesCalorimeterSD::Initialize(G4HCofThisEvent* hce)
{
  //G4cout <<  "Initializing Hits collection " << collectionName[0] << " with Sensitive detector: " << SensitiveDetectorName << G4endl;
  // Create hits collection
  fHitsCollection = new DREMTubesCalorimeterHitsCollection(SensitiveDetectorName, collectionName[0]); 
  ...
  //  fHitsCollection->insert(new DREMTubesCalorimeterHit());
  //}
...
G4bool DREMTubesCalorimeterSD::ProcessHits(G4Step* step, 
                                     G4TouchableHistory*)
{
  ...
  { 
    ...
    if (SiPMTower >-1)                                        // save hits only in SiPM-mounted modules
    {
      ...
      auto hit  = new DREMTubesCalorimeterHit();

      if ( ! hit ) {
        G4ExceptionDescription msg;
        msg << "Cannot access hit " ; 
        G4Exception("DREMTubesCalorimeterSD::ProcessHits()",
...
void DREMTubesCalorimeterSD::EndOfEvent(G4HCofThisEvent*)
{
